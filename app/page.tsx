...
export default function ChatWidget() {
  ...
  const [chatHeight, setChatHeight] = useState(() => {
    if (typeof window !== "undefined") {
      ...
      return isMobileDevice
        ? Math.min(window.innerHeight * 0.9, window.innerHeight - 50)
        ...
        try {
          if (messageData?.type === "init") {
            if (messageData.session_id) {
              ...
              console.warn(
                "[Chatbot] Received init message without session_id",
              );
            }
            ...
            const handleConversationsReceived = (conversationsData: any[]) => {
              ...
              try {
                if (Array.isArray(conversationsData)) {
                  const formattedConversations = conversationsData
                    .map((conv: any) => {
                      ...
                      if (!conv) {
                        console.warn(
                          "[Chatbot] Null conversation found, skipping",
                        );
                        return null;
                      }
                      ...
                      if (
                        ...
                        console.warn(
                          "[Chatbot] No valid conversation ID found for:",
                          conv,
                        );
                        return null;
                      }
                ...
                console.error(
                  "[Chatbot] Error processing conversations:",
                  error,
                );
                setConversations([]);
                setConversationsCache([]);
            ...
            if (
              messageData.response &&
              (messageData.response.message || messageData.response.ai_message)
            ) {
              // Safely handle cards data
              let safeCards = undefined;
              if (
                messageData.response.cards &&
                Array.isArray(messageData.response.cards)
              ) {
                safeCards = messageData.response.cards.filter(
                  (card: any) => card && typeof card === "object",
                );
              }
              ...
              const responseMessage: Message = {
                ...
                content:
                  ...
                  messageData.response.ai_message ||
                  "Response received",
                role: "webhook",
                timestamp: new Date(),
                type: "text",
            ...
            console.log(
              "[Chatbot] Continue shopping action - staying in conversation",
            );
  ...
  useEffect(() => {
    const checkMobile = () => {
      ...
      if (mobile) {
        ...
        setChatHeight(
          Math.min(window.innerHeight * 0.92, window.innerHeight - 50),
        );
      } else {
  ...
  const handleAddToCart = (
    ...
    console.log("[Chatbot] handleAddToCart called with:", {
      card,
      selectedVariant,
      quantity,
    });
    ...
    if (!variantId) {
      ...
      const errorMessage: Message = {
        id: `error-${Date.now()}`,
        content:
          "Sorry, I couldn't add this product to your cart. Please try selecting a different variant or contact support.",
        role: "webhook",
        timestamp: new Date(),
        type: "text",
      };
    ...
    <Fragment>
      ...
      {!isOpen && (
        <div className="fixed bottom-6 right-6 z-50">
          ...
          >
            <MessageCircle
              className={`h-6 w-6 transition-all duration-300 ${isHovered ? "scale-110" : ""}`}
            />
